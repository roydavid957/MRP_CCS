...
def get_data(src_path, model_name, device):
    ...
    for _,row in tqdm(df.iterrows(),total=df.shape[0]):
      translated_story = {'StoryID': row['StoryID'], 'Sentence1': '', 'Sentence2': '', 'Sentence3': '', 'Sentence4': '', 'Sentence5': '',
                          'RandomFinalSentence': ''}
      sentences = [row['Sentence1'],row['Sentence2'],row['Sentence3'],row['Sentence4'],row['Sentence5'],row['RandomFinalSentence']]
      for idx,sample_text in enumerate(sentences):
        ...
        if idx+1 <= 5:
          ...
          translated_story['RandomFinalSentence'] = translated_batch
